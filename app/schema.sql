...
create table public.profiles (
  ...
  -- That's cached so we don't need to dig deep into auth.users user metadata
...
begin
  ...
  values (new.id, new.raw_user_meta_data::jsonb #>> '{sub}');
  return new;
end;
$$;
...
-- Allow for linking multiple wallets to a single user/Discord ID
create table public.wallets (
  id bigint generated by default as identity primary key,
  profile uuid references public.profiles not null,
  eth citext unique not null
);

alter table public.wallets enable row level security;

create policy "Only users can view their own Ethereum wallets."
  on wallets for select
  using ( auth.uid() = profile );

create policy "Users can insert their own Ethereum wallets."
  on wallets for insert
  with check ( auth.uid() = profile );

create policy "Users can update own Ethereum wallets."
